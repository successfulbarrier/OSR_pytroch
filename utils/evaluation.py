...
class Evaluation(object):
    ...
    def _precision_recall(self, average) -> (float, float):
        ...
        assert len(self.predict) == len(self.label)
        #-------------------------------------------------#
        #   你可以在调用`precision_recall_fscore_support`函数时，
        #   通过设置`zero_division`参数来控制警告的行为。你可以将`zero_division`参数设置为一个值，
        #   例如1，以避免出现精确度和F1分数为0的警告。
        #-------------------------------------------------#
        precision, recall, _, _ = precision_recall_fscore_support(self.label, self.predict, average=average, zero_division=1)
